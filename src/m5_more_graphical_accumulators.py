...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
        new_square = rg.Square(rg.Point(circle.center.x + circle.radius * k, circle.center.y + circle.radius * k),
                               circle.radius * 2)
...
def run_test_draw_circles_from_rectangle():
    ...
    p1 = rg.Point(400, 250)
    p2 = rg.Point(440, 325)
    rec1 = rg.Rectangle(p1, p2)
    rec1.fill_color = 'green'
    rec1.outline_color = 'black'
    rec1.outline_thickness = 5
    ...
    rec1.outline_color = 'red'
    rec1.outline_thickness = 3
    draw_circles_from_rectangle(8, 3, rec1, window1)
    window1.close_on_mouse_click()
    ...
    p2 = rg.Point(350, 280)
    rec1 = rg.Rectangle(p1, p2)
    rec1.fill_color = 'yellow'
    rec1.outline_color = 'brown'
    rec1.outline_thickness = 5
    draw_circles_from_rectangle(6, 10, rec1, window2)
    window2.close_on_mouse_click()
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for k in range(m):
        circle1 = rg.Circle(
            rg.Point(rectangle.get_center().x - rectangle.get_width() / 2 -
                     rectangle.get_height() * (2 * k + 1) / 2,
    ...
    for k in range(n):
        circle2 = rg.Circle(rg.Point(rectangle.get_center().x,
                                     rectangle.get_center().y - rectangle.get_height() / 2 -
                                     rectangle.get_width() * (2 * k + 1) / 2), rectangle.get_width() / 2)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    for k in range(n):
        x1 = rectangle1.get_center().x - rectangle1.get_width() * k / 2
        y1 = rectangle1.get_center().y + rectangle1.get_height() * k / 2
        x2 = rectangle2.get_center().x - rectangle1.get_width() * k / 2
        y2 = rectangle2.get_center().y + rectangle1.get_height() * k / 2
